...
void main() {
  setup_adc(
      ADC_CLOCK_INTERNAL); // Seleccionamos el reloj interno RC para ADC 35kHz.
  ...
  while (true) {
    ...
    printf(lcd_putc, "ADC = %4.3f",
           valor);    // Imprimir "valor" como 4 enteros 3 decimales.
    lcd_gotoxy(1, 2); // Cursos en Fila 2, Comumna 1.
    printf(lcd_putc, "Voltaje = %4.3f",
           voltaje); // Imprimir "conversion" como 4 enteros 3 decimales.
    delay_ms(500);
  }
}
